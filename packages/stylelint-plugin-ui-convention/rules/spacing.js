...
var _a = stylelint.utils,
  report = _a.report,
  validateOptions = _a.validateOptions,
  ruleMessages = _a.ruleMessages;
var RULE_PROPERTIES = {
  margin: true,
  'margin-top': true,
  'margin-right': true,
...
var plugin = stylelint.createPlugin(
  ...
  function (primaryOption, secondaryOptions, context) {
    return function (postcssRoot, postcssResult) {
      ...
      var validOptions = validateOptions(postcssResult, ruleName, {
        ...
        possible: function (value) {
          ...
          if (
            ...
            var key = Object.keys(primaryOption).find(function (key) {
              return !VARIABLE_REG.test(key);
            });
            if (key) {
              postcssResult.warn(
                '\u914D\u7F6E\u683C\u5F0F\u9519\u8BEF,'
      ...
      // 根据文件类型加工配置变量名
      var option = (0, index_1.processOption)(
      ...
      postcssRoot.walkDecls(function (decl) {
        ...
        var invalidValues = values.filter(function (val) {
          return !option[val];
        });
        // 全是预设变量 -> 正确 -> 返回
        ...
        // 有非预设值 -> 没有对应的预设变量 -> 报错
        var inValidValue = invalidValues.find(function (val) {
          return !revertedOption[val];
        });
        if (inValidValue) {
          report({
            ruleName: ruleName,
        ...
        var variables = validValues.map(function (validValue) {
          return revertedOption[validValue];
        });
        // 是预设的值 -> 有预设的变量 -> 开启了修复 -> 直接修复
        if (context.fix) {
          var reg = new RegExp(
            validValues
              .map(function (validValue) {
                return '('.concat(validValue, ')');
              })
              .join('|'),
            'g'
          );
          var replacer = function (match) {
            return revertedOption[match];
          };
