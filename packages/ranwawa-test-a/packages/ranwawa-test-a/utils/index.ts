...
export const processOption = (primaryOption: TOption | true, postcssRoot: Root): TOption => {
...
export const createAllowedListPluginFactory = ({ruleName, variableReg, defaultOption, ruleProperty}: IPluginFactory) => {
	console.log(98, ruleName)
	return stylelint.createPlugin(ruleName, (primaryOption, secondaryOptions, context) => {
		return (postcssRoot, postcssResult) => {
				...
				ruleName,
				{
					...
					possible: (value) => {
						...
						if (typeof primaryOption === 'object' && primaryOption !== null && !Array.isArray(primaryOption)) {
							const key = Object.keys(primaryOption).find(key => !variableReg.test(key))

							if (key) {
								postcssResult.warn(`配置格式错误,${key}必须符合${variableReg}`, { stylelintType: 'invalidOption', });
								return false
							}
						...
						postcssResult.warn(`配置格式错误,配置只能是对象如:${JSON.stringify(defaultOption)}`, { stylelintType: 'invalidOption', });
						return false
					},
					optional: true
				}
			...
			// 根据文件类型加工配置变量名
			const option = processOption(primaryOption === true ? defaultOption : primaryOption, postcssRoot)
